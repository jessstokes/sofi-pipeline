...
;
;     SoFI manual: 
;     http://www.eso.org/sci/facilities/lasilla/instruments/sofi/doc.html
;
;     xtalk correction: 
;     http://www.eso.org/~gfinger/hawaii_1Kx1K/crosstalk_rock/crosstalk.html
;     https://www.eso.org/sci/facilities/lasilla/instruments/sofi/tools/reduction/sofi_scripts/crosstalk.cl
;
;     special dome flat fields and illumination correction: 
;     http://www.eso.org/sci/facilities/lasilla/instruments/sofi/tools/reduction/flat_fielding.html
;
; CALLING SEQUENCE
...
;
; DEPENDENCIES:
;    IDL Astronomy User's Library (not included)
;    Markwardt Library cmtotal (not included)
;
;    Packaged
;    by JR:
;     define_references.pro
;     do_phot.pro
;     flat_field.pro
;     get_target_positions.pro
;     grab_im_coords.pro
;     illum_cor.pro
;     mk_cube.pro
;     obslog.pro
...
;
;    by D. Lafreniere:
;     subarr.pro (D. Lafreniere)
;     tvdl (D. Lafreniere)
;     load8colors (D. Lafreniere)
;     medfits.pro (D. Lafreniere)
;     filelist.pro (D. Lafreniere)
;     nbrlist.pro (D. Lafreniere)
;
;    by Other:
;     esosxpar.pro
;     trim.pro
...
if ~keyword_set(skip_xtalk) then begin
   if ~file_test(xtalk_dir) then spawn, 'mkdir '+xtalk_dir
...
test_apertures=1
aps=[2,2.5,3,3.5,4,4.5,5,5.5,6,6.5,7,7.5,8]
nap=n_elements(aps)
mean_rms=fltarr(nap)
...
for i=0,nap-1 do begin
   do_phot,outdir,sky_dir,aperture_type=aperture_type,aperture_size=aps[i],aperture_array=aperture_array,sky_annulus=sky_annulus,cntrd_type=0,tname=tname,rms_cut=rms_cut,mskip=mskip,avg=avg,return_ref_rms=return_ref_rms,allow_src=allow_src,test_apertures=1
